...
(function () {
  ...
  function resetForm() {
    ...
    if (uploadLabel) {
      uploadLabel.textContent =
        "Click to upload the designed flyer (JPG or PNG, max 5MB)";
    }
  ...
  async function populateStaffOptions(staffSelect) {
    ...
    try {
      const { data, error } = await window.supabaseClient
        .from("profiles")
        .select(
          "id, first_name, last_name, department, position, is_active"
        )
        .eq("is_active", true)
      ...
      staffSelect.innerHTML =
        ...
        staff
          .map((s) => {
            ...
            return `
              <option
                value="${escapeAttr(s.id)}"
                data-name="${escapeAttr(name)}"
                data-dept-position="${escapeAttr(deptPosition)}"
              >
                ${escapeAttr(name)}
              </option>
            `;
      ...
      console.error(
        "[VSAS Birthday] Failed to load staff list:",
        err
      );
  ...
  async function handlePublishSubmit(event) {
    ...
    const staffSelect =
      document.getElementById("birthdayStaffSelect");

    const nameField =
      document.getElementById("birthdayDisplayName");

    const deptPositionField =
      document.getElementById("birthdayDeptPosition");
    ...
    if (!selectedFlyerFile) {
      markFieldInvalid(
        "birthdayFlyerInput",
        "birthdayFlyerError",
        "Upload a JPG or PNG flyer under 5MB."
      );
    ...
    try {
       ...
       * IMPORTANT:
       *
       * Get the currently authenticated Supabase user before attempting
       * the birthday_celebrations INSERT.
       *
       * The birthday_celebrations RLS policy requires:
       *
       *   created_by = auth.uid()
       *
       * Therefore we must have a real authenticated user UUID here.
       */
      const {
        data: userData,
        error: userError,
      ...
      /*
       * Never allow the INSERT to continue with:
       *
       *   created_by: null
       *
       * because the RLS policy requires created_by to match auth.uid().
       */
      if (!currentUserId) {
        throw new Error(
          "Your login session could not be verified. Please sign in again before publishing a birthday celebration."
        );
      }

      // 1. Upload the flyer as-is. VSAS never generates or edits this image.
      const safeName =
        selectedFlyerFile.name.replace(
          /[^a-zA-Z0-9._-]/g,
          "_"
        );

      const path =
        `${staffId}/${Date.now()}-${safeName}`;
      ...
      if (!flyerUrl) {
        throw new Error(
          "Could not resolve a public URL for the uploaded flyer."
        );
      }
      ...
      const {
        ...
        .insert({
           ...
           * It must match auth.uid() for the INSERT RLS policy.
           */
      ...
      try {
        await window.supabaseClient.functions.invoke(
          "manage-vsas",
          {
            body: {
              ...
              body:
                message ||
                `Join us in celebrating ${displayName}'s birthday today!`,
        ...
        console.warn(
          "[VSAS Birthday] Celebration published, but the companion notification failed:",
          notifyErr
        );
      }
      ...
      notify(
        `Birthday celebration for ${displayName} was published.`,
        "success"
      );
      ...
      console.error(
        "[VSAS Birthday] Failed to publish celebration:",
        err
      );

      showBirthdayAlert(
        err?.message ||
          "Could not publish this celebration. Please try again."
      );
    } finally {
  ...
  function showBirthdayAlert(message) {
    const alertBox =
      document.getElementById("birthdayAlert");

    const alertText =
      document.getElementById("birthdayAlertText");
  ...
  function ensureOverlayDom() {
    ...
    overlay.innerHTML = `
      ...
      <canvas
        class="birthday-overlay__confetti"
        id="birthdayConfettiCanvas"
      ></canvas>

      <button
        type="button"
        class="birthday-overlay__close"
        id="closeBirthdayOverlayBtn"
      ...
      <div class="birthday-overlay__content">
        <div class="birthday-overlay__headline">
          HAPPY BIRTHDAY!
        </div>

        <div
          class="birthday-overlay__name"
          id="birthdayOverlayName"
        ></div>

        <div class="birthday-overlay__flyer-frame">
          <img
            id="birthdayOverlayFlyer"
            class="birthday-overlay__flyer"
            alt="Birthday flyer"
          />
        </div>

        <p
          class="birthday-overlay__message"
          id="birthdayOverlayMessage"
        ></p>
      </div>
    `;
  ...
  function playCelebration(record) {
    ...
    const flyerEl =
      document.getElementById(
        "birthdayOverlayFlyer"
      );

    const messageEl =
      document.getElementById(
        "birthdayOverlayMessage"
      );
